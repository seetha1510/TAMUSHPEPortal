...
class EmployeesController < ApplicationController
  ...
  def show
    ...
    @email = if @user_profile.user_display_email_status
               '*************'
                ...
                User.find(@user_profile.user_id).user_email
             end
  ...
  def create
    ...
    @user_profile_id = User.get_current_user(current_account).id
    ...
    if @employer_object
      ...
      if @employee_object.save
        redirect_to employee_path(User.get_current_user(current_account).id)
      else
